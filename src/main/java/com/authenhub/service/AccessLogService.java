...
//            accessLog.setSessionId(request.getSession().getId());
    ...
    public Map<String, Object> getAccessStats(LocalDateTime start, LocalDateTime end) {
        ...
        // Get daily visits
        List<Map<String, Object>> dailyVisits = accessLogRepository.countByDay(start, end);
        stats.put("dailyVisits", dailyVisits);
        
        // Get browser stats
        List<Map<String, Object>> browserStats = accessLogRepository.countByBrowser(start, end);
        stats.put("browserStats", browserStats);
        
        // Get device type stats
        List<Map<String, Object>> deviceStats = accessLogRepository.countByDeviceType(start, end);
        stats.put("deviceStats", deviceStats);
        
        // Get top endpoints
        List<Map<String, Object>> topEndpoints = accessLogRepository.getTopEndpoints(start, end);
        stats.put("topEndpoints", topEndpoints);
        
        // Get top users
        List<Map<String, Object>> topUsers = accessLogRepository.getTopUsers(start, end);
